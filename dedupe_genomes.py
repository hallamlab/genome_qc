...
mq_df = sorted_df[keep_cols].query("Completeness >= @comp & Contamination <= @cont & qscore >= @qsmin & pass_BARRNAP != False").drop_duplicates('Genome_Id')
...
for gen_id in list(mq_df['Genome_Id']):
	...
	gen_tot_df = sorted_df.query("Genome_Id == @gen_id & Completeness == @mq_comp & Contamination == @mq_cont & pass_BARRNAP != False")
