...
case $1 in
    ...
    -rd) # docker
        ...
        docker run -it --rm \
            -u $(id -u):$(id -g) \
            --mount type=bind,source="$HERE/data",target="/app/data"\
            --mount type=bind,source="$HERE/scratch/docker",target="/app/ws"\
            --mount type=bind,source="$HERE/scratch/docker/cache",target="/app/.snakemake"\
            --workdir="/app" \
